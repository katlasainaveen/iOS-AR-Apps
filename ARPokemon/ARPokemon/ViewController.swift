...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        // can also use ARWorldTrankingConfiguration() - will detect both planes and images
        let configuration = ARImageTrackingConfiguration()
        
        if let imageToTrack = ARReferenceImage.referenceImages(inGroupNamed: "Pokemon-cards", bundle: .main) {
            // if wee use ARWorldTrankingConfiguration() - use .detectImages
    ...
    func renderer(_ renderer: SCNSceneRenderer, nodeFor anchor: ARAnchor) -> SCNNode? {
        ...
        if let imageAnchor = anchor as? ARImageAnchor {
            ...
            if let name = imageAnchor.referenceImage.name {
                ...
                print(docName)
                if let pokeScene = SCNScene(named: "art.scnassets/\(docName).scn") {
